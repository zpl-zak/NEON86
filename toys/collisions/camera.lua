...
function updateCamera(dt)
	...
	camera.rhs = lookAt:col(1)
		...
		local testCamCols = false
			camera.pos = camera.pos + camera.vel

    for _, side in pairs(roomSides) do
        ok, move = side:intersectsSphere(camera.pos, 1, Vector(camera.vel:x(), 0, 0))

        if ok then
            camera.pos += move
        end

        ok, move = side:intersectsSphere(camera.pos, 1, Vector(0, camera.vel:y(), 0))

        if ok then
            camera.pos += move
        end

        ok, move = side:intersectsSphere(camera.pos, 1, Vector(0, 0, camera.vel:z()))
