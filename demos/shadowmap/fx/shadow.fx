...
float ApplyPoissonSampling(float4 seed, float r, float amt, float spread, float2 shadowCoord, sampler2D shadowMap, float depth)
{
    for (int i=0; i<4; i++)
    {
        seed.w = i;
        ...
        if (tex2D(shadowMap, shadowCoord + poissonDisk[idx]/spread).r < depth + shadowEpsilon)
            r -= amt;
    }
